...
import { FileText, Clock, User, Activity, Database, Download } from "lucide-react";
import { TableVirtuoso } from "react-virtuoso";

const VirtuosoTable = /* @__PURE__ */ __name((props) => <table {...props} className="w-full text-left" />, "VirtuosoTable");
const VirtuosoTableBody = React.forwardRef((props, ref) => <tbody {...props} ref={ref} className="divide-y divide-gray-100 dark:divide-gray-800" />);

const AuditLogs = /* @__PURE__ */ __name(() => {
  const { auditLogs, fetchAuditLogs, users, fetchUsers } = useAppStore();
  const renderPrettyValue = /* @__PURE__ */ __name((value) => {
    if (value === null || value === void 0) return <span className="text-gray-400 italic">N/A</span>;
    ...
    if (Array.isArray(value)) {
      ...
      return <div className="flex flex-wrap gap-2 mt-2">
          {value.map((v, i) => <span key={i} className="px-2.5 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-md text-[11px] font-bold border border-blue-100 dark:border-blue-800 break-words max-w-full">
    ...
    if (isDiffObject) {
      return <div className="space-y-2 mt-2 w-full">
          {keys.map((k) => {
        const fieldDiff = value[k];
        return <div key={k} className="bg-gray-50 dark:bg-[#0F172A] p-3 rounded-lg border border-gray-200 dark:border-gray-800 flex flex-col gap-2">
    ...
    return <div className="bg-gray-50 dark:bg-[#0F172A] p-3 rounded-lg border border-gray-200 dark:border-gray-800 space-y-2 mt-2 w-full">
        {keys.map((k) => <div key={k} className="flex flex-col sm:flex-row sm:justify-between border-b border-gray-200 dark:border-gray-700/50 last:border-0 pb-2 last:pb-0 gap-1 sm:gap-4">
  ...
  // Stable set of user names — grows as logs load, never shrinks when filter is applied
  const seenUserNames = useRef(new Set());
  const [stableUserNames, setStableUserNames] = useState([]);

  // Accumulate names from users store
  useEffect(() => {
  ...
  // Accumulate names from audit logs only when viewing unfiltered results
  useEffect(() => {
  ...
  const USERS_LIST = stableUserNames;
  ...
  // Re-fetch audit logs whenever any filter changes
  useEffect(() => {
  ...
  const exportToCSV = /* @__PURE__ */ __name(() => {
    ...
    link.setAttribute("download", `audit_logs_report_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }, "exportToCSV");
  return <div className="flex flex-col h-[calc(100vh-100px)] space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <PageHeader
    title="Audit Logs"
  ...
  />
    ...
    itemContent={(_index, log) => <>
                ...
                <div className="mt-3 space-y-2 lg:hidden">
                  ...
                  <div className="flex sm:hidden flex-col gap-2">
                    ...
                    <div className="flex items-center mt-1">
                       <span className={`px-2 py-0.5 rounded-[4px] text-[10px] font-bold ${log.action === "LOGIN" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30" : log.action === "CREATE" ? "bg-green-100 text-green-700 dark:bg-green-900/30" : log.action === "DELETE" ? "bg-red-100 text-red-700 dark:bg-red-900/30" : "bg-amber-100 text-amber-700 dark:bg-amber-900/30"}`}>
      ...
      /* 3. Action (Medium+) */
              ...
              <td className="hidden md:table-cell px-6 py-4">
                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${log.action === "LOGIN" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30" : log.action === "CREATE" ? "bg-green-100 text-green-700 dark:bg-green-900/30" : log.action === "DELETE" ? "bg-red-100 text-red-700 dark:bg-red-900/30" : "bg-amber-100 text-amber-700 dark:bg-amber-900/30"}`}>
                  {log.action ? log.action.charAt(0) + log.action.slice(1).toLowerCase() : "Unknown"}
                </span>
              </td>
    ...
    >
                    View Details
                  </button> : <span className="text-gray-400 italic text-[11px]">No data</span>}
              </td>
            </>}
    components={{
      Table: VirtuosoTable,
  ...
  />
        ...
        {filteredLogs.length === 0 && <div className="px-6 py-12 text-center text-gray-500 absolute inset-0 flex flex-col items-center justify-center">
  ...
  >
          <div className="bg-gray-50 dark:bg-[#0F172A] p-4 sm:p-6 rounded-xl overflow-y-auto max-h-[65vh] border border-gray-200 dark:border-gray-800 shadow-inner">
            {selectedLog.details && Object.keys(selectedLog.details).length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {Object.entries(selectedLog.details).map(([key, value]) => {
    const formattedKey = key.replace(/([A-Z])/g, " $1").replace(/^./, (str) => str.toUpperCase()).trim();
    return <div key={key} className="bg-white dark:bg-[#1E293B] p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
              ...
              </div> : <div className="flex flex-col items-center justify-center py-12 text-gray-400">
