...
const StockCheckReports = /* @__PURE__ */ __name(() => {
  ...
  const handleReject = /* @__PURE__ */ __name(async () => {
    if (!approvalModal) return;
    try {
  ...
  const handleDeleteReport = /* @__PURE__ */ __name(async (id) => {
  ...
  const downloadPDF = /* @__PURE__ */ __name((report) => {
    let doc;
    try {
  ...
  />
      ...
      {loading && stockCheckReports.length === 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => <Card key={i} className="p-4 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
        ...
        </div> : stockCheckReports.length === 0 ? <Card className="p-12 flex flex-col items-center justify-center text-center">
          ...
          {stockCheckReports.map((report) => <Card key={report._id || report.id} className="p-4 hover:shadow-md transition-shadow bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`text-[10px] font-bold tracking-wider px-2 py-1 rounded ${report.status === "Completed" ? "text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20" : report.status === "Pending Approval" ? "text-orange-500 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20" : report.status === "Approved" ? "text-blue-500 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20" : "text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20"}`}>
                  {report.status}
                </span>
              </div>
  ...
  >
    ...
    wide
    footer={<div className="flex flex-wrap justify-end gap-3 w-full">
              <div className="flex gap-2 mr-auto">
                <Btn label="Download PDF" icon={Download} outline onClick={() => downloadPDF(selectedReport)} />
              </div>
              
              {selectedReport.status === "Pending Approval" && ["Super Admin", "Director", "AGM", "Inventory Manager", "Project Manager"].includes(role || "") && <div className="flex gap-2">
                  <Btn
      label="Reject"
      variant="ghost"
  ...
  >
          <div className="space-y-6">
            ...
            {selectedReport.approvalReason && <div className={`p-4 rounded-xl border ${selectedReport.status === "Approved" ? "bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-800" : "bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-800"}`}>
            ...
            <div className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
              <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                  {selectedReport.items.map((item, idx) => <tr key={`${item.sku}-${idx}`} className="bg-white dark:bg-gray-900">
                      ...
                      <td className={`px-4 py-2 text-right font-bold ${item.variance > 0 ? "text-blue-500 dark:text-blue-400" : item.variance < 0 ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"}`}>
  ...
  >
          <div className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {approvalModal.type === "approve" ? "Approving this shortage will update the system inventory to match the physical count. Please provide a reason/remark." : "Rejecting this audit will keep the system inventory unchanged. Please provide a reason for rejection."}
            </p>
            <textarea
    value={reason}
