...
import { scrollToError, todayStr } from "../utils";
import { toast } from "react-hot-toast";
const PublicMaterialRequirement = /* @__PURE__ */ __name(() => {
  const { submitPublicMaterialRequirement, actionLoading, inventory, catalogue, api, fetchResource, settings } = useAppStore();
  useEffect(() => {
    fetchResource("public/inventory", 1, 2e3, true);
    fetchResource("public/catalogue", 1, 2e3, true);
    ...
    const checkedItems = await Promise.all(
      (form.items || []).map(async (item) => {
        ...
        return {
          ...
          unit: item.unit || invItem?.unit || catalogueItem?.uom || item.unit,
          availableInStock,
          remainingQty,
          status
        };
  ...
  return <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8 font-sans">
    ...
    required
  />
                {form.requesterName === "Other" && <div className="mt-2">
                    <Field
  ...
  />
    ...
    required
  />
                {form.project === "Other" && <div className="mt-2">
                    <Field
  ...
  />
            ...
            <div className="space-y-4 pt-6 border-t border-gray-100 dark:border-gray-800">
              ...
              <div className="space-y-4">
                {form.items.map((item, idx) => <div key={idx} className="bg-gray-50/30 dark:bg-gray-800/20 p-5 rounded-2xl border border-gray-100 dark:border-gray-800 relative group transition-all hover:border-orange-200 dark:hover:border-orange-900/30">
