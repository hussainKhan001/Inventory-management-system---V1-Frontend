...
const ListManager = ({ 
  ...
  <div className="p-5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/10 border border-gray-100 dark:border-gray-800 transition-all hover:shadow-xs">
    ...
    <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto no-scrollbar p-1">
      ...
      {items.length === 0 && (
        <p className="text-[10px] text-gray-400 italic mt-1">No items added yet</p>
      )}
    </div>
  </div>
);
...
export const SettingsPage = () => {
  ...
  // Custom states for master list lists
  ...
  const removeItemFromList = async (listKey: 'projects' | 'requesters' | 'categories' | 'units' | 'workTypes', item: string) => {
    const updatedList = (settings[listKey] || []).filter((i: string) => i !== item);
    const updatedSettings = { ...settings, [listKey]: updatedList };
    setSettings(updatedSettings);
    try {
      await saveSettings(updatedSettings);
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      {/* POLICIES TAB */}
      {activeTab === "policies" && (
        <Card className="p-0 border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-300 overflow-hidden">
      ...
      {activeTab === "master-data" && (
        <Card className="p-6 space-y-6 border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="p-5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/10 border border-gray-100 dark:border-gray-800 md:col-span-2">
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                ...
                {(!settings.companies || settings.companies.length === 0) && (
                  <p className="text-center py-8 text-xs text-gray-400 italic">No companies configured yet</p>
                )}
              </div>
            </div>
          </div>
