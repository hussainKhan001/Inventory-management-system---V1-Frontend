...
export const PublicMRTracking = () => {
  ...
  const getTimelineSteps = (status: string) => {
    ...
    if (isRejected) {
      // Find where it was rejected (approximate based on data if possible, but default to AGM/Store)
      steps[1].status = 'completed'; // Assuming it passed store if it's rejected, or we just mark step 2 as failed
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0F172A] flex flex-col font-sans transition-colors duration-200">
      {/* Header */}
              ...
              >
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                          mr.status === 'Approved' || mr.status === 'Allocated' ? 'bg-green-50 text-green-600 dark:bg-green-500/10 dark:text-green-400' :
                          mr.status === 'Rejected' ? 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400' :
                      ...
                      {getTimelineSteps(mr.status).map((step, i) => {
                        ...
                        return (
                          <div key={step.id} className="flex items-start gap-4 group">
                            <div className={`relative z-10 w-12 h-12 rounded-2xl flex items-center justify-center transition-colors shadow-sm ${
                              isCompleted ? 'bg-green-500 text-white' :
                              isCurrent ? 'bg-blue-500 text-white ring-4 ring-blue-500/20' :
                              isFailed ? 'bg-red-500 text-white' :
                              'bg-white dark:bg-[#1E293B] border-2 border-gray-200 dark:border-gray-700 text-gray-400'
                            }`}>
                ...
                <div className="bg-white dark:bg-[#1E293B] rounded-3xl border border-gray-100 dark:border-gray-800 p-6 sm:p-8 shadow-sm">
                  ...
                  <div className="space-y-3">
                    {mr.items && mr.items.map((item: any, i: number) => (
                      <div key={i} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800/50">
                        <div>
