...
const WriteOffPage = /* @__PURE__ */ __name(() => {
  ...
  const handleApprove = /* @__PURE__ */ __name(async (id) => {
        ...
        <div className="overflow-x-auto no-scrollbar">
          <table className="w-full text-left border-collapse table-fixed min-w-[750px]">
            ...
            <tbody className="divide-y divide-[#E8ECF0] dark:divide-gray-800">
              {loading && writeOffs.length === 0 ? [...Array(5)].map((_, i) => <tr key={i}>
                    ...
                    <td className="px-3 py-2.5"><Skeleton className="h-7 w-full" /></td>
                    <td className="px-3 py-2.5 text-right"><Skeleton className="h-7 w-full ml-auto" /></td>
                  </tr>) : writeOffs.map((wo) => <tr key={wo.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right space-x-2">
                      {hasPermission("APPROVE_WRITE_OFF") && wo.status === "Pending" && <>
                            <Btn
    icon={Check}
    small
    color="green"
  ...
  />
                      ...
                      {hasPermission("DELETE_WRITE_OFF") && <Btn
    icon={Trash2}
    small
    outline
    color="red"
  ...
  >
      ...
      {writeOffsPagination && <Pagination
