...
const SiteManager = /* @__PURE__ */ __name(({ sites, onUpdate, isSuperAdmin }) => {
  const [newSite, setNewSite] = useState({ siteName: "", siteCode: "" });
  
  const handleAdd = () => {
    if (!newSite.siteName.trim()) return;
    onUpdate([...(sites || []), { siteName: newSite.siteName.trim(), siteCode: newSite.siteCode.trim() }]);
    setNewSite({ siteName: "", siteCode: "" });
  };
  ...
  return (
    <div className={`p-5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/10 border border-gray-100 dark:border-gray-800 transition-all ${!isSuperAdmin ? "opacity-60" : "hover:shadow-xs"} md:col-span-2`}>
      <h4 className="text-[12px] font-bold text-gray-700 dark:text-gray-300 tracking-widest mb-3 flex items-center gap-2">
        <Building className="w-4 h-4 text-primary" />
        Sites & Projects (with Codes)
      </h4>
      ...
      {isSuperAdmin && (
        <div className="flex gap-2 mb-3">
          <input
            ...
            placeholder="Site Name (e.g. Garden City Store)"
            className="flex-[2] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-primary/20 transition-all"
          />
          <input
            type="text"
            ...
            placeholder="Code (e.g. GC)"
            className="flex-1 sm:w-32 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-primary/20 transition-all"
          />
          <button
            onClick={handleAdd}
            className="p-2 bg-primary text-white rounded-xl hover:bg-primary/90 transition-all flex items-center justify-center shrink-0 w-9 h-9 cursor-pointer"
      ...
      <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto no-scrollbar p-1">
        {(sites || []).map((site, idx) => (
          <div key={idx} className={`flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-full text-[11px] font-medium text-gray-600 dark:text-gray-400 transition-all ${isSuperAdmin ? "group hover:border-red-200 hover:bg-red-50 dark:hover:bg-red-900/10" : ""}`}>
            <span>{site.siteName}</span>
            {site.siteCode && <span className="text-[9px] text-primary font-bold uppercase tracking-wider ml-0.5">({site.siteCode})</span>}
            {isSuperAdmin && (
              <button
                onClick={() => handleRemove(idx)}
                className="text-gray-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 cursor-pointer ml-1"
              >
                <CloseIcon className="w-3 h-3" />
              </button>
            )}
        ...
        {(!sites || sites.length === 0) && <p className="text-[10px] text-gray-400 italic mt-1">No sites/projects added yet</p>}
      </div>
    </div>
  );
}, "SiteManager");
...
const SettingsPage = /* @__PURE__ */ __name(() => {
      ...
      {activeTab === "master-data" && <Card className="p-6 space-y-6 border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <SiteManager
              ...
              onUpdate={async (newSites) => {
                const updatedSettings = { ...settings, sites: newSites };
                setSettings(updatedSettings);
                try { await saveSettings(updatedSettings); toast.success("Sites updated"); } catch {}
              }}
            />
  ...
  />
    ...
    onRemove={async (item) => {
      const found = gstRates.find((r) => (r.label || `${r.rate}% GST`) === item);
      if (found) await removeGSTRate(found._id);
    }}
    items={gstRates.length ? gstRates.map((r) => r.label || `${r.rate}% GST`) : ["0% GST","5% GST","12% GST","18% GST","28% GST"]}
    disabled={!isSuperAdmin}
  />
